    ...
    internal partial class HostItem : DynamicObject, IExpando, IDynamic, IEnumVARIANT, ICustomQueryInterface, IScriptMarshalWrapper
    {
        ...
        private object InvokeHostMember(string name, BindingFlags invokeFlags, object[] args, object[] bindArgs, CultureInfo culture, out bool isCacheable)
        {
            ...
            if (invokeFlags.HasFlag(BindingFlags.InvokeMethod))
            {
                ...
                var property = target.Type.GetScriptableProperty(name, GetCommonBindFlags(), MiscHelpers.GetEmptyArray<object>(), this.engine.DisableCaseInsensitivePropertyLookups);
        ...
        private object GetHostProperty(string name, BindingFlags invokeFlags, object[] args, object[] bindArgs, CultureInfo culture, out bool isCacheable)
        {
            ...
            var property = target.Type.GetScriptableProperty(name, invokeFlags, bindArgs, this.engine.DisableCaseInsensitivePropertyLookups);
            if (property != null)
            {
            ...
            if (target.Type.GetScriptableProperties(name, invokeFlags, this.engine.DisableCaseInsensitivePropertyLookups).Any())
            {
        ...
        private object SetHostProperty(string name, BindingFlags invokeFlags, object[] args, object[] bindArgs, CultureInfo culture)
        {
            ...
            var property = target.Type.GetScriptableProperty(name, invokeFlags, bindArgs.Take(bindArgs.Length - 1).ToArray(), this.engine.DisableCaseInsensitivePropertyLookups);
            if (property != null)
            {
